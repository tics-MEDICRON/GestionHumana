...
if ($USUARIO->getTipoEnObjeto() == "Colaborador") {
    $resultado = HistorialSarlaft::getListaEnObjetos("concat(nombres,' ',apellidos) = '$USUARIO'", null);
    for ($i = 0; $i < count($resultado); $i++) {
        ...
        $lista .= "<td><a href='presentacion/historial/documentos/{$academica->getEvaluacionPdf()}' target='_blank' title='Ver su certficado'><img src='presentacion/img/pdf.png'></a></td>";
        $lista .= '</tr>';
    }
} else {

    $resultado = HistorialSarlaft::getListaEnObjetos($filtro, null);
    for ($i = 0; $i < count($resultado); $i++) {
        $academica = $resultado[$i];
        $lista .= '<tr>';
        $lista .= "<td>{$academica->getId()}</td>";
        ...
        $lista .= "<td><a href='presentacion/historial/documentos/{$academica->getEvaluacionPdf()}' target='_blank' title='Ver su certficado'><img src='presentacion/img/pdf.png'></a></td>";
        $lista .= '<td>';
        $lista .= "<a href='principal.php?CONTENIDO=presentacion/historial/historialSarlaftFormulario.php&accion=Modificar&id={$academica->getId()}'><img src='presentacion/img/modificar.png'></a>";
...
<center>
    ...
    <center>
        ...
        if ($USUARIO->getTipoEnObjeto() == "Colaborador") {
            ...
            echo "<form method='post' action='principal.php?CONTENIDO=presentacion/historial/historialSarlaft.php' class='d-flex'>
        ...
        <th><a href='principal.php?CONTENIDO=presentacion/historial/historialSarlaftFormulario.php'><img src='presentacion/img/adicionar.png'></a></th>
        </tr>
...
<button type="button" class="btn btn-danger" value="Cancelar" onclick="location='principal.php?CONTENIDO=presentacion/hojaDeVida/hvDatosActualizarComplementaria.php'">Cancelar</button>  -->
        ...
        <script type="text/javascript">
            function eliminar(id) {
                ...
                if (respuesta) location = "principal.php?CONTENIDO=presentacion/historial/historialSarlaftActualizar.php&accion=Eliminar&id=" + id;
            }
        </script>
